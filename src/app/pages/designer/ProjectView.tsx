  ...
  return (
    <div className="editorial-page">
      ...
      <div style={{ paddingTop: 80 }}>
        ...
        {/* Project banner */}
        <div style={{ padding: bannerPadding, borderBottom: '1px solid #D9DFDA', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: isMobile ? 'wrap' : 'nowrap', gap: isMobile ? 16 : 0 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
              <button onClick={() => navigate('/designer')} style={{ fontFamily: "'Poppins', sans-serif", fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#70767A', background: 'none', border: 'none', cursor: 'pointer' }}>
                ← Projects
              </button>
            ...
            <p style={{ fontFamily: "'Poppins', sans-serif", fontSize: 10, letterSpacing: '0.08em', color: '#70767A', marginBottom: 12 }}>
              ...
              return (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ display: 'flex', gap: 3 }}>
                    {ZONES.map(z => (
                      <div key={z.id} style={{ width: 24, height: 3, borderRadius: 1.5, background: itemsByZone[z.id]?.length > 0 ? '#21493F' : '#D9DFDA' }} />
                    ))}
                  </div>
                  <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: 8, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#70767A' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', flexShrink: 0 }}>
            {(project.status === 'configuring' || project.status === 'revisions') && (
              <>
                <button onClick={handleAddItems} className="flex items-center gap-2" style={{ fontFamily: "'Poppins', sans-serif", fontSize: 9, fontWeight: 600, letterSpacing: '0.22em', textTransform: 'uppercase', background: 'none', color: '#1B1B1B', border: '1px solid #A7B9AE', padding: '10px 18px', cursor: 'pointer', transition: 'all 0.3s' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = '#1B1B1B'; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = '#A7B9AE'; }}>
                ...
                <button
                  ...
                  style={{ fontFamily: "'Poppins', sans-serif", fontSize: 9, fontWeight: 600, letterSpacing: '0.22em', textTransform: 'uppercase', background: projectItems.length === 0 ? '#D9DFDA' : '#1B1B1B', color: projectItems.length === 0 ? '#70767A' : '#FFFFFF', border: 'none', padding: '10px 18px', cursor: projectItems.length === 0 ? 'not-allowed' : 'pointer', transition: 'background 0.3s' }}
                  onMouseEnter={e => { if (projectItems.length > 0) e.currentTarget.style.background = '#21493F'; }}
                  onMouseLeave={e => { if (projectItems.length > 0) e.currentTarget.style.background = '#21493F'; }}
                >
          ...
          <div style={{ padding: isMobile ? '24px 16px' : isTablet ? '28px 24px' : '40px 48px' }}>
                          ...
                          {zoneItems.map(item => (
                            <div key={item.id} style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', gap: 14, padding: '14px 16px', background: STATUS_BG[item.status], border: `1px solid ${STATUS_DOT[item.status]}20`, flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                {item.clientNote && (
                                  <div style={{ fontFamily: "'Poppins', sans-serif", fontSize: 9, color: '#21493F', marginTop: 4, fontStyle: 'italic' }}>Client: "{item.clientNote}"</div>
                                )}
                                {item.designerNote && noteItemId !== item.id && (
                                  <div style={{ fontFamily: "'Poppins', sans-serif", fontSize: 9, color: '#70767A', marginTop: 4, fontStyle: 'italic' }}>Note: "{item.designerNote}"</div>
                                )}
                                {item.status === 'flagged' && noteItemId !== item.id && (
                                  <button
                                    onClick={() => { setNoteItemId(item.id); setNoteText(item.designerNote ?? ''); }}
                                    style={{ fontFamily: "'Poppins', sans-serif", fontSize: 8, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#21493F', background: 'none', border: '1px solid rgba(33,73,63,0.3)', padding: '3px 8px', cursor: 'pointer', marginTop: 6 }}
                                  >
                                ...
                                {noteItemId === item.id && (
                                  <div style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
                                    <input
                                      ...
                                      style={{ flex: 1, minWidth: 120, background: '#F1F2EF', border: '1px solid #D9DFDA', color: '#1B1B1B', padding: '5px 8px', fontFamily: "'Poppins', sans-serif", fontSize: 9, outline: 'none' }}
                                    />
                                    <button onClick={() => saveDesignerNote(item.id)} style={{ fontFamily: "'Poppins', sans-serif", fontSize: 8, background: '#21493F', color: '#FFFFFF', border: 'none', padding: '4px 10px', cursor: 'pointer' }}>Save</button>
                                    <button onClick={() => { setNoteItemId(null); setNoteText(''); }} style={{ background: 'none', border: '1px solid #A7B9AE', color: '#70767A', padding: '4px 6px', cursor: 'pointer', fontSize: 10 }}>✕</button>
                                  </div>
                                )}
                              </div>
                              <div className="flex items-center gap-3">
                                ...
                                {confirmDeleteId === item.id ? (
                                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                                    <button
                                      ...
                                      style={{ fontFamily: "'Poppins', sans-serif", fontSize: 7.5, letterSpacing: '0.1em', background: '#9F3A2F', color: '#FFFFFF', border: 'none', padding: '3px 8px', cursor: 'pointer' }}
                                    >
                                      Remove
                                    </button>
                                    <button
                                      onClick={() => setConfirmDeleteId(null)}
                                      style={{ background: 'none', border: '1px solid #A7B9AE', color: '#70767A', padding: '3px 6px', cursor: 'pointer', fontFamily: "'Poppins', sans-serif", fontSize: 7.5 }}
                                    >
                                      ✕
                                    </button>
                                  </div>
            ...
            {projectItems.length === 0 && (
              <div style={{ textAlign: 'center', padding: '64px 0' }}>
                <p style={{ fontFamily: "'Inter', sans-serif", fontStyle: 'italic', fontSize: 18, color: '#70767A', marginBottom: 20 }}>No items configured yet.</p>
                <button onClick={handleAddItems} style={{ fontFamily: "'Poppins', sans-serif", fontSize: 9, fontWeight: 600, letterSpacing: '0.25em', textTransform: 'uppercase', background: '#21493F', color: '#FFFFFF', border: 'none', padding: '12px 24px', cursor: 'pointer' }}>
